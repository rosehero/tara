...
import com.sondertara.common.annotation.Export;
import com.sondertara.common.annotation.Import;
import com.sondertara.common.enums.ImportErrorCodeEnum;
import com.sondertara.common.enums.Null;
import com.sondertara.common.model.ImportBaseDTO;
import com.sondertara.common.model.ImportFailureDTO;
import com.sondertara.common.model.ImportResultDTO;
...
public class ExcelUtil {
     ...
     * @return 文件url
     *//*
    private static String uploadFailedFile(Workbook workbook) {
        String fileName = String.format("insurance/importFail/导入失败记录-%s.xls", new SimpleDateFormat("yyyyMMddHHmmss").format(new Date()));
        return uploadWorkbook(workbook, fileName);
    }*/
     ...
     * @return 文件url
     *//*
    public static String uploadWorkbook(Workbook workbook, String fileName) {
        FileStorageRepository fileStorageRepository = new FileStorageRepositoryAliyunImpl();
        ((FileStorageRepositoryAliyunImpl) fileStorageRepository).init();
        try {
            fileStorageRepository.put(fileName, toByteArray(workbook), null);
        } catch (IOException e) {
            e.printStackTrace();
            return null;
        }
        return ImageUtil.getAbsoluteImageUrl(fileName);
    }*/
...
//                cellValue = cell.getCellFormula();
    ...
    private static Object getCellNumberValue(Cell cell, Field field) {
        Object cellValue = getCellValue(cell);
        if (cellValue instanceof Number && field.isAnnotationPresent(Import.class)) {
            String nf = field.getAnnotation(Import.class).numberFormat();
            cellValue = getNumberFormat(nf).format(cellValue);
        }
        return cellValue;
    }
    ...
    private static Object getCellValue(Cell cell, Field field) {
        ...
        if (cell.getRowIndex() == 0 || field.getType() == String.class) {
            return getCellStringValue(cell);
        }
        if (Number.class.isAssignableFrom(field.getType())) {
            return getCellNumberValue(cell, field);
        }
        return getCellValue(cell);
    }
